...
class ResNet1D(nn.Module):
    ...
    Pararmeters:
        ...
        base_filters: number of filters in the first several Conv layer, it
        will double at every 4 layers
        kernel_size: width of kernel
        stride: stride of kernel moving
        groups: set target to 1 as ResNeXt
    ...
    def __init__(self, in_channels, base_filters, kernel_size, stride, groups,
        ...
        for i_block in range(self.n_block):
            ...
            if is_first_block:
                ...
                in_channels = int(base_filters * 2 ** (
                            (i_block - 1) // self.increasefilter_gap))
    ...
    def forward(self, x):
        ...
        for i_block in range(self.n_block):
            ...
            if self.verbose:
                print('i_block: {0}, in_channels: {1}, '
                      'out_channels: {2}, downsample: {3}'.format(
                        i_block, net.in_channels, net.out_channels,
                        net.downsample))
            out = net(out)
            if self.verbose:
                print(out.shape)
