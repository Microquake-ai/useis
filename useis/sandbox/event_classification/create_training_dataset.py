...
def create_spectrograms(stream: Stream, event_type: str, end_time: UTCDateTime,
    ...
    for sampling_rate in sampling_rates:
        ...
        for window_length in window_length_seconds:
            ...
            st_trimmed = st_trimmed.taper(max_percentage=0.1)
...
def create_blast_training_set(stream: Stream, event: Event):
    ...
    while end_time <= blast_end_time:
        create_spectrograms(stream, 'blast', end_time, event.magnitudes[-1].mag)
        end_time += 1
...
def create_seismic_event_training_set(stream: Stream, event: Event):
    ...
    create_spectrograms(stream, event_type, end_signal, event.magnitudes[-1].mag)


def create_noise_training_set(stream: Stream, event: Event):
    end_signal = stream[0].stats.starttime + 12
    create_spectrograms(stream, 'noise', end_signal, event.magnitudes[-1].mag,
...
def create_training_dataset(context_trace):
    ...
    if event_type_lookup[event.event_type] == 'blast':
        create_blast_training_set(st, event)

    elif event_type_lookup[event.event_type] == 'seismic event':
        site_codes = np.array([site.code for site in
                               classifier_project.inventory.sites])
        arrivals = np.array([arrival.pick.waveform_id.site for arrival in
                             event.preferred_origin().arrivals if
                             arrival.phase == 'P'])
        site_codes = np.unique(np.hstack((site_codes, arrivals)))

        if st[0].stats.site not in site_codes:
            return
        ...
        if event.preferred_magnitude() is None:
            if event.magnitudes[-1].mag == -999:
                return
        st.traces = traces

        start_trace, end_trace = find_end_signal(st.copy(), event)
        durations.append(end_trace - start_trace)
        magnitudes.append(event.magnitudes[-1].mag)

        create_seismic_event_training_set(st, event)

    elif 'noise' in event_type_lookup[event.event_type]:
        create_noise_training_set(st, event)
...
if __name__ == "__main__":
    ...
    # with tqdm(total=len(mseed_files)) as pbar:
        # use imap_unordered to apply the function to the data in parallel
    tqdm(pool.imap(create_training_dataset, mseed_files),
